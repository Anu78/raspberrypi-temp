...
class JoystickReader():
  ...
  def assignAction(self, direction, action): 
    if not callable(action): raise TypeError("action must be a callable function.")
    if direction not in self.fns: raise ValueError("valid directions are up, down, left, and right.")
